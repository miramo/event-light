...
const PLAYER = 'mpv --ytdl-format=best --no-video';
...
export class App {
  ...
  async onMessage(message: IMessage) {
    ...
    switch (true) {
      ...
      case /deployement .+ failed/gim.test(text):
      ...
      case /just passed staging step/gim.test(text):
      ...
      case /is now deployed in production/gim.test(text):
